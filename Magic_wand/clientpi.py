#!/usr/bin/env python
"""clientpi.py:
Client python script for Magic Wand SuperProject of Embedded Interface Design Course.
This Python script runs natively on the "Client-Raspberry Pi" of the Magic Wand SuperProject,
and uses the boto3 library to interface with AWS.
The user controls the Client-Raspberry Pi using the following voice commands:
-"Identify"
-"Correct"
-"Wrong"
The operation of the script is as follows:
-wait for the user to press a physical button connected to the Client-Raspberry Pi
-one the button is pressed, the script requests a command from the user via audio
-When the command "identify" is received the script takes a photo using the installed camera.
-The photo is then sent to AWS via S3 for processing.
-A label is generated by AWS and presented to the user.
-The user must reply with "Correct" or "Wrong" in response to the label
-An audio file will be played indicating that the generated label was 
correct or incorrect for the image taken
-All commands (valid or invalid) are sent to SQS for further processing by the Server Raspberry Pi.
-All generated labels and their results are also sent to SQS for further processing by the Server Raspberry Pi.
